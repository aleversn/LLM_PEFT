...
try:
    tokenizer = AutoTokenizer.from_pretrained(r"/root/autodl-tmp/models/qwen2.5-3b-instruct", trust_remote_code=True)
    config = AutoConfig.from_pretrained(r"/root/autodl-tmp/models/qwen2.5-3b-instruct", trust_remote_code=True)
    trainer = Trainer(tokenizer=tokenizer, config=config, from_pretrained=r"/root/autodl-tmp/models/qwen2.5-3b-instruct", \
        reward_from_pretrained=r"/root/autodl-tmp/models/text2vec-base-multilingual", loader_name='LLM_RLHF',\
        data_path='Wiki_Humans_RL_100', ratio_for_rlhf=-1.0, max_length=1024, batch_size=4,  task_name='Wiki-humans-glm4-sen-jaccard-100' + '_' + datetime.datetime.now().strftime("%Y%m%d-%H%M%S"))
